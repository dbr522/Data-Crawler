...
path = 'E:/201602 WeiboAPI Improve/Data/'
filename = 'Weibo_Guiyang_2012.txt'
tokenFile = 'token.txt'
coordFile = 'wuzhen_500.txt'
searchRange = '354'    # Radius is 500m
sleeper = 24    # Need to be decided
            ...
            if contents is not None:
                for content in contents:
                    ...
                    if result is not None:
                        sql = 'insert into Table_1 values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
...
def deleteRep():
    mssql = MSSQL(host, user, pw, database)
    (cur, conn) = mssql.connect()
    query = "SELECT DISTINCT * INTO [Weibo Checkin].[dbo].[Temp1] FROM [Weibo Checkin].[dbo].[Table_Wuzhen] \
    WHERE convert(int,cast(lat as float)) > 20 AND convert(int,cast(lat as float)) < 60 AND lat NOT Like '' \
    DROP TABLE [Weibo Checkin].[dbo].[Table_Wuzhen] \
    SELECT DISTINCT * INTO [Weibo Checkin].[dbo].[Table_Wuzhen] FROM [Weibo Checkin].[dbo].[Temp1]\
    DROP TABLE [Weibo Checkin].[dbo].[Temp1]"
    cur.execute(query)
    conn.commit()
